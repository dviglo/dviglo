...
bool ParticleEffect::Load(const XmlElement& source)
{
    ...
    if (source.IsNull())
    {
        DV_LOGERROR("Can not load particle effect from null XML element");
        return false;
    }
    ...
    if (source.HasChild("material"))
    {
        ...
        if (GetAsyncLoadState() == ASYNC_LOADING)
            DV_RES_CACHE.BackgroundLoadResource<Material>(loadMaterialName_, true, this);
    }
...
bool ParticleEffect::Save(XmlElement& dest) const
{
    if (dest.IsNull())
    {
        DV_LOGERROR("Can not save particle effect to null XML element");
        return false;
    }
