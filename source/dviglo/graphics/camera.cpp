...
void Camera::RegisterObject()
{
    ...
    DV_ACCESSOR_ATTRIBUTE("Zoom", GetZoom, SetZoom, 1.0f, AM_DEFAULT);
    ...
    DV_ATTRIBUTE("View Override Flags", viewOverrideFlags_.AsInteger(), VO_NONE, AM_DEFAULT);
    DV_ACCESSOR_ATTRIBUTE("Projection Offset", GetProjectionOffset, SetProjectionOffset, Vector2::ZERO, AM_DEFAULT);
    DV_ACCESSOR_ATTRIBUTE("Reflection Plane", GetReflectionPlaneAttr, SetReflectionPlaneAttr,
        Vector4(0.0f, 1.0f, 0.0f, 0.0f), AM_DEFAULT);
    DV_ACCESSOR_ATTRIBUTE("Clip Plane", GetClipPlaneAttr, SetClipPlaneAttr, Vector4(0.0f, 1.0f, 0.0f, 0.0f),
        AM_DEFAULT);
    DV_ACCESSOR_ATTRIBUTE("Use Reflection", GetUseReflection, SetUseReflection, false, AM_DEFAULT);
    ...
    if (frustumDirty_)
    {
        ...
        else
        {
            ...
            if (!orthographic_)
                ...
                frustum_.define_ortho(orthoSize_, aspectRatio_, zoom_, GetNearClip(), GetFarClip(), GetEffectiveWorldTransform());
        }
...
Frustum Camera::GetSplitFrustum(float nearClip, float farClip) const
{
    ...
    else
    {
        if (!orthographic_)
            ...
            ret.define_ortho(orthoSize_, aspectRatio_, zoom_, nearClip, farClip, GetEffectiveWorldTransform());
    }
...
Frustum Camera::GetViewSpaceFrustum() const
{
    ...
    else
    {
        if (!orthographic_)
            ...
            ret.define_ortho(orthoSize_, aspectRatio_, zoom_, GetNearClip(), GetFarClip());
    }
...
Frustum Camera::GetViewSpaceSplitFrustum(float nearClip, float farClip) const
{
    ...
    else
    {
        if (!orthographic_)
            ...
            ret.define_ortho(orthoSize_, aspectRatio_, zoom_, nearClip, farClip);
    }
