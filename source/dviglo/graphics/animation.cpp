...
bool Animation::begin_load(Deserializer& source)
{
    ...
    {
        DV_LOGERROR(source.GetName() + " is not a valid animation file");
        return false;
    }
    ...
    String jsonName = replace_extension(GetName(), ".json");

    SharedPtr<JSONFile> jsonFile(DV_RES_CACHE->GetTempResource<JSONFile>(jsonName, false));
    if (jsonFile)
    {
...
bool Animation::Save(Serializer& dest) const
{
    ...
    if (!triggers_.Empty() || HasMetadata())
    {
        ...
        if (destFile)
        {
            ...
            DV_LOGWARNING("Can not save animation trigger data when not saving into a file");
    }
