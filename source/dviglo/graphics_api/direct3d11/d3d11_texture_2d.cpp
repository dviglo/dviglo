...
{
    if (!GParams::is_headless() && object_.ptr_)
    {
...
bool Texture2D::SetData_D3D11(unsigned level, int x, int y, int width, int height, const void* data)
{
    ...
    if (usage_ == TEXTURE_DYNAMIC)
    {
        ...
        HRESULT hr = graphics.GetImpl_D3D11()->GetDeviceContext()->Map((ID3D11Resource*)object_.ptr_, subResource, D3D11_MAP_WRITE_DISCARD, 0,
            &mappedData);
        ...
        else
        {
            ...
            graphics.GetImpl_D3D11()->GetDeviceContext()->Unmap((ID3D11Resource*)object_.ptr_, subResource);
        }
    }
    else
    {
        ...
        graphics.GetImpl_D3D11()->GetDeviceContext()->UpdateSubresource((ID3D11Resource*)object_.ptr_, subResource, &destBox, data,
            rowSize, 0);
    }
...
bool Texture2D::GetData_D3D11(unsigned level, void* dest) const
{
    ...
    srcBox.front = 0;
    srcBox.back = 1;
    graphics.GetImpl_D3D11()->GetDeviceContext()->CopySubresourceRegion(stagingTexture, 0, 0, 0, 0, srcResource,
...
bool Texture2D::Create_D3D11()
{
    ...
    if (multiSample_ > 1 && !graphics.GetImpl_D3D11()->CheckMultiSampleSupport(textureDesc.Format, multiSample_))
    {
        multiSample_ = 1;
        autoResolve_ = false;
    }
    ...
    textureDesc.SampleDesc.Quality = graphics.GetImpl_D3D11()->GetMultiSampleQuality(textureDesc.Format, multiSample_);
    ...
    if (usage_ == TEXTURE_DEPTHSTENCIL && multiSample_ > 1 && graphics.GetImpl_D3D11()->GetDevice()->GetFeatureLevel() < D3D_FEATURE_LEVEL_10_1)
        textureDesc.BindFlags &= ~D3D11_BIND_SHADER_RESOURCE;
    ...
    if (multiSample_ > 1 && autoResolve_)
    {
        ...
        HRESULT hr = graphics.GetImpl_D3D11()->GetDevice()->CreateTexture2D(&textureDesc, nullptr, (ID3D11Texture2D**)&resolveTexture_);
        if (FAILED(hr))
        {
    ...
    if (textureDesc.BindFlags & D3D11_BIND_SHADER_RESOURCE)
    {
        ...
        hr = graphics.GetImpl_D3D11()->GetDevice()->CreateShaderResourceView(viewObject, &resourceViewDesc,
    ...
    else if (usage_ == TEXTURE_DEPTHSTENCIL)
    {
        ...
        hr = graphics.GetImpl_D3D11()->GetDevice()->CreateDepthStencilView((ID3D11Resource*)object_.ptr_, &depthStencilViewDesc,
        ...
        if (graphics.GetImpl_D3D11()->GetDevice()->GetFeatureLevel() >= D3D_FEATURE_LEVEL_11_0)
        {
