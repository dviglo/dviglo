...
void CrowdManager::RegisterObject()
{
    ...
    DV_ATTRIBUTE("Navigation Mesh", navigationMeshId_, 0, AM_DEFAULT | AM_COMPONENTID);
    DV_ACCESSOR_ATTRIBUTE("Filter Types", GetQueryFilterTypesAttr, SetQueryFilterTypesAttr,
    ...
    DV_ACCESSOR_ATTRIBUTE("Obstacle Avoidance Types", GetObstacleAvoidanceTypesAttr, SetObstacleAvoidanceTypesAttr,
        Variant::emptyVariantVector, AM_DEFAULT)
...
unsigned short CrowdManager::GetIncludeFlags(unsigned queryFilterType) const
{
    if (queryFilterType >= numQueryFilterTypes_)
        DV_LOGWARNINGF("Query filter type %d is not configured yet, returning the default include flags initialized by dtCrowd",
            queryFilterType);
...
{
    if (queryFilterType >= numQueryFilterTypes_)
        DV_LOGWARNINGF("Query filter type %d is not configured yet, returning the default exclude flags initialized by dtCrowd",
            queryFilterType);
    const dtQueryFilter* filter = GetDetourQueryFilter(queryFilterType);
...
bool CrowdManager::create_crowd()
{
    ...
    {
        DV_LOGERROR("Could not initialize DetourCrowd");
        return false;
    }
    ...
    if (recreate)
    {
        ...
        for (unsigned i = 0; i < agents.Size(); ++i)
        {
            ...
            {
                DV_LOGWARNINGF("CrowdManager: %d crowd agents orphaned", agents.Size() - i);
                break;
            }
        }
    }
...
void CrowdManager::OnSceneSet(Scene* scene)
{
    ...
    if (scene)
    {
        if (scene != node_)
        {
            DV_LOGERROR("CrowdManager is a scene component and should only be attached to the scene node");
            return;
        }
