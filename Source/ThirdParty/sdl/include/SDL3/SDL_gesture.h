...
// Этот файл был удалён из SDL 3 и перенесён в отдельный репозиторий
...
Gesture_SaveDollarTemplate(Gesture_ID gestureId, SDL_RWops *dst)
{
    ...
    for (i = 0; i < GestureNumTouches; i++) {
        ...
        for (j = 0; j < touch->numDollarTemplates; j++) {
            ...
            // Dviglo: Закомментировано. https://github.com/libsdl-org/SDL_gesture/issues/8
...
// Dviglo: Добавлено. https://github.com/libsdl-org/SDL_gesture/issues/8
static void GestureRemoveDollarTemplate_one(GestureTouch* inTouch, int index)
{
    ...
    if (inTouch->numDollarTemplates > 1) {
        inTouch->dollarTemplate = (GestureDollarTemplate *)SDL_realloc(inTouch->dollarTemplate,
...
// Dviglo: Добавлено. https://github.com/libsdl-org/SDL_gesture/issues/8
int Gesture_RemoveDollarTemplate(Gesture_ID gestureId)
{
    int i,j,ret = 0;
    for (i = 0; i < GestureNumTouches; i++) {
        GestureTouch* touch = &GestureTouches[i];
        for (j = 0; j < touch->numDollarTemplates; j++) {
            // Gesture IDs are stored as 32bit, so check the low bits only
            if ((touch->dollarTemplate[j].hash & 0xffffffff) == (gestureId & 0xffffffff)) {
                GestureRemoveDollarTemplate_one(touch, j);
                ret = 1;
            }
        }
    }
...
// Dviglo: Добавлено. https://github.com/libsdl-org/SDL_gesture/issues/8
void Gesture_RemoveAllDollarTemplates(void)
{
    int i;
    for (i = 0; i < GestureNumTouches; i++) {
        GestureTouch* touch = &GestureTouches[i];
...
{
    // Dviglo: Закомментировано. https://github.com/libsdl-org/SDL_gesture/issues/9
